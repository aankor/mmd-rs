...
impl<R: Read> VertexReader<R> {
  pub fn new(mut header: HeaderReader<R>) -> Result<VertexReader<R>, Error> {
  ...
  pub fn next_vertex<I: TryFrom<i8> + TryFrom<i16> + TryFrom<i32>>(&mut self) -> Result<Option<Vertex<I>>, Error> {
    if self.remaining == 0 {
      return Ok(None)
    }
...
impl<'a, R: Read, I: TryFrom<i8> + TryFrom<i16> + TryFrom<i32>> FallibleIterator for VertexIterator<'a, R, I> {
  type Item = Vertex<I>;
  type Error = Error;

  fn next(&mut self) -> Result<Option<Self::Item>, Self::Error> {
    self.reader.next_vertex()
  }
}
